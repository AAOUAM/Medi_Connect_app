...
def patient_dashboard():
    ...
    patient_id = session['id_fonctionnel']
    patient = mongo_service.get_patient_by_id(patient_id)
    medecin_filter = request.args.get('medecin', '').strip().lower()
    date_filter = request.args.get('date', '').strip()

    # Récupérer les consultations déjà formatées
    consultations = mongo_service.get_consultations_by_patient_id(patient_id)

    # Filtrage
    if medecin_filter:
        consultations = [c for c in consultations if medecin_filter in c.get('nom_medecin', '').lower()]

    if date_filter:
        consultations = [c for c in consultations if c.get('date', '') == date_filter]

    return render_template('patient_dashboard.html', consultations=consultations, patient=patient)
